...
movimientos_file = st.sidebar.file_uploader("movimientos_juan.csv", type=["csv"])
productos_file = st.sidebar.file_uploader("productos_juan.csv (opcional)", type=["csv"])
rop_file = st.sidebar.file_uploader("rop_sugerido_juan.csv (opcional)", type=["csv"])

try:
    df = load_movimientos(Path("data/movimientos_juan.csv"), movimientos_file)
except FileNotFoundError:
    st.error("No se encontro data/movimientos_juan.csv. Subi el archivo en la barra lateral.")
    st.stop()

resumen, ventas_1y_full, stock_real_diario, quantiles, ventana_inicio, max_date = compute_rop(df)
rop_sugerido = load_rop_sugerido(Path("data/rop_sugerido_juan.csv"), rop_file)
...
productos = load_productos(Path("data/productos_juan.csv"), productos_file)
if not productos.empty:
    enrich_cols = [c for c in ["codigo", "descrip", "prec_vta", "depto", "familia", "st_real", "punt_ped"] if c in productos.columns]
