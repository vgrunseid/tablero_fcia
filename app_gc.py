...
st.set_page_config(page_title="Tablero ROP - GC", layout="wide")
...
def load_movimientos(data_source: Path | None, uploaded_file) -> pd.DataFrame:
    if uploaded_file is not None:
        ...
        if data_source is None or not data_source.exists():
            raise FileNotFoundError("No se encuentra movimientos_anio_movil.csv")
...
@st.cache_data(show_spinner=False)
def load_productos(data_source: Path | None, uploaded_file) -> pd.DataFrame:
    if uploaded_file is not None:
        prod_df = pd.read_csv(uploaded_file, encoding="latin1", low_memory=False)
    else:
        if data_source is None or not data_source.exists():
            return pd.DataFrame()
        prod_df = pd.read_csv(data_source, encoding="latin1", low_memory=False)

    prod_df.columns = prod_df.columns.str.strip().str.lower()
    if "codigo" not in prod_df.columns:
        return pd.DataFrame()

    prod_df["codigo"] = prod_df["codigo"].astype(str).str.strip()
    return prod_df
...
with col_title:
    st.title("Tablero ROP - GC Reposicion")

st.markdown('<div class="section-sep"></div>', unsafe_allow_html=True)
st.markdown("## Consulta")

st.sidebar.header("Datos")
movimientos_file = st.sidebar.file_uploader("movimientos_anio_movil.csv", type=["csv"])
productos_file = st.sidebar.file_uploader("snapshot_producto.csv (opcional)", type=["csv"])
rop_file = st.sidebar.file_uploader("rop_sugerido (opcional)", type=["csv"])

try:
    df = load_movimientos(Path("/Users/vivi/Downloads/GC_REPOSICION/DATOS/movimientos_anio_movil.csv"), movimientos_file)
except FileNotFoundError:
    st.error("No se encontro movimientos_anio_movil.csv. Subi el archivo en la barra lateral.")
    st.stop()

resumen, ventas_1y_full, stock_real_diario, quantiles, ventana_inicio, max_date = compute_rop(df)
rop_sugerido = load_rop_sugerido(Path("/Users/vivi/Downloads/GC_REPOSICION/DATOS/rop_sugerido_intradia_20260325_1950.csv"), rop_file)
extra_cols = ["stock_fijado", "stock_actual", "AhorroCapitalAjustado", "ExcesoCapitalHolgado"]
...
productos = load_productos(Path("/Users/vivi/Downloads/GC_REPOSICION/DATOS/snapshot_producto.csv"), productos_file)
if not productos.empty:
    enrich_cols = [c for c in ["codigo", "descrip", "prec_vta", "depto", "familia", "st_real", "punto_ped_sif"] if c in productos.columns]
    if "codigo" in enrich_cols:
        productos_enrich = productos[enrich_cols].drop_duplicates(subset=["codigo"], keep="first")
        resumen = resumen.merge(productos_enrich, on="codigo", how="left", suffixes=("", "_prod"))
        if "descrip_prod" in resumen.columns:
            resumen["descrip"] = resumen["descrip"].fillna(resumen["descrip_prod"])
            resumen = resumen.drop(columns=["descrip_prod"], errors="ignore")
